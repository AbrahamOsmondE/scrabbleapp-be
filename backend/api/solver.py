class SolveState:
    ...
    def legal_move(self, word, last_pos):
        ...
        if self.direction == 'across':
            curr['start'] = [row, col+1]
        else:
            curr['start'] = [row+1, col]

        curr['points'] = self.board.count_points(
            word, self.direction, curr['start'], curr['end'])
            ...
            for anchor_pos in anchors:
                if self.board.is_filled(self.before(anchor_pos)):
                    ...
                    while self.board.is_filled(self.before(scan_pos)):
                        ...
                        partial_word = self.board.get_tile(
                            scan_pos) + partial_word
                    ...
                    self.before_part("", self.dictionary.root,
                                     anchor_pos, limit)

    def find_all_options_empty(self, words):
        for word in words:
            start_row, start_col = 7, 7-len(word)+1
            end_row, end_col = 7, 7
            while start_col <= 7:
                curr = {'direction': 'across',
                        'word': word,
                        'end': [end_row, end_col],
                        'start': [start_row, start_col]}

                curr['points'] = self.board.count_points(
                    word, 'across', curr['start'], curr['end'])
